...
void EV_Crowbar(event_args_t* args)
{
	...
	if (iBigSwing)
	{
		...
		if (EV_IsLocal(idx))
		{
			V_PunchAxis(0, shakeX * 2);
			V_PunchAxis(1, shakeY * 2);
		}
		// Shake End
		if (hitSomething)
		{
			gEngfuncs.pEventAPI->EV_WeaponAnimation(CROWBAR_BIG_SWING_HIT, 0);
		}
		else
		{
			gEngfuncs.pEventAPI->EV_WeaponAnimation(CROWBAR_BIG_SWING_MISS, 0);
		}

		gEngfuncs.pEventAPI->EV_PlaySound(idx, origin, CHAN_WEAPON, "weapons/fireaxe_big_miss.wav", 1, ATTN_NORM, 0, PITCH_NORM);
	}
	else
	{
		// Shake Start
		...
		if (hitSomething)
		{
			...
			{
			case 0:
				gEngfuncs.pEventAPI->EV_WeaponAnimation(CROWBAR_ATTACK1HIT, 0); break;
			case 1:
				gEngfuncs.pEventAPI->EV_WeaponAnimation(CROWBAR_ATTACK2HIT, 0); break;
			case 2:
				gEngfuncs.pEventAPI->EV_WeaponAnimation(CROWBAR_ATTACK3HIT, 0); break;
			}
		}
		else
		{
			...
			{
			case 0:
				gEngfuncs.pEventAPI->EV_WeaponAnimation(CROWBAR_ATTACK1MISS, 0); break;
			case 1:
				gEngfuncs.pEventAPI->EV_WeaponAnimation(CROWBAR_ATTACK2MISS, 0); break;
			case 2:
				gEngfuncs.pEventAPI->EV_WeaponAnimation(CROWBAR_ATTACK3MISS, 0); break;
			}

			switch (g_iClub % 2)
			{
			case 0: gEngfuncs.pEventAPI->EV_PlaySound(idx, origin, CHAN_WEAPON, "weapons/fireaxe_miss1.wav", 1, ATTN_NORM, 0, PITCH_NORM); break;
			case 1: gEngfuncs.pEventAPI->EV_PlaySound(idx, origin, CHAN_WEAPON, "weapons/fireaxe_miss2.wav", 1, ATTN_NORM, 0, PITCH_NORM); break;
			}
		}
...
void EV_EjectShell(event_args_t* args)
{
	int idx;
	vec3_t origin;
	vec3_t angles;
...
//	    AR16 START
//======================
void EV_FireAR16(event_args_t* args)
{
	int idx;
	vec3_t origin;
	vec3_t angles;
	...
	if (EV_IsLocal(idx))
	{
		// Add muzzle flash to current weapon model
		EV_MuzzleFlash();
		gEngfuncs.pEventAPI->EV_WeaponAnimation(AR16_FIRE1 + gEngfuncs.pfnRandomLong(0, 2), 2);
	...
	{
	case 0:
		gEngfuncs.pEventAPI->EV_PlaySound(idx, origin, CHAN_WEAPON, "weapons/ar16_fire1.wav", 1, ATTN_NORM, 0, 94 + gEngfuncs.pfnRandomLong(0, 0xf));
		break;
	case 1:
		gEngfuncs.pEventAPI->EV_PlaySound(idx, origin, CHAN_WEAPON, "weapons/ar16_fire2.wav", 1, ATTN_NORM, 0, 94 + gEngfuncs.pfnRandomLong(0, 0xf));
		break;
	}
...
void EV_FireAR162(event_args_t* args)
{
	int idx;
	vec3_t origin;
...
//	   CROWBAR START
//======================
// NULL Event - don't use this. G-Cont.
//======================
//	   CROWBAR END 
//======================
...
void EV_FireCrossbow( event_args_t *args )
{
	...
	shell = gEngfuncs.pEventAPI->EV_FindModelIndex("models/338shell.mdl");// brass shell
	...
	gEngfuncs.pEventAPI->EV_PlaySound(idx, origin, CHAN_WEAPON, "weapons/xbow_fire1.wav", 1, ATTN_NORM, 0, 94 + gEngfuncs.pfnRandomLong(0, 0xf));
