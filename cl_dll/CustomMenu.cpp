...
#define CUSTOMMENU_TITLE_Y YRES(32)
#define CUSTOMMENU_TOPLEFT_BUTTON_X XRES(40)
#define CUSTOMMENU_TOPLEFT_BUTTON_Y YRES(80)
#define CUSTOMMENU_BUTTON_SIZE_X XRES(124)
#define CUSTOMMENU_BUTTON_SIZE_Y YRES(24)
#define CUSTOMMENU_BUTTON_SPACER_Y YRES(8)
#define CUSTOMMENU_WINDOW_X XRES(176)
#define CUSTOMMENU_WINDOW_Y YRES(80)
#define CUSTOMMENU_WINDOW_SIZE_X XRES(424)
#define CUSTOMMENU_WINDOW_SIZE_Y YRES(312)
#define CUSTOMMENU_WINDOW_TEXT_X XRES(150)
#define CUSTOMMENU_WINDOW_TEXT_Y YRES(80)
#define CUSTOMMENU_WINDOW_NAME_X XRES(150)
#define CUSTOMMENU_WINDOW_NAME_Y YRES(8)
...
CCustomMenu::CCustomMenu(int iTrans, int iRemoveMe, int x, int y, int wide, int tall) : CMenuPanel(iTrans, iRemoveMe, x, y, wide, tall)
{
	...
	// Create the title
	Label* pLabel = new Label("", CUSTOMMENU_TITLE_X, CUSTOMMENU_TITLE_Y);
	...
	pLabel->setText(gHUD.m_TextMessage.BufferedLocaliseTextString("#Title_CUSTOM MENU HUD"));

	// Create the Scroll panel
	m_pScrollPanel = new CTFScrollPanel(CUSTOMMENU_WINDOW_X, CUSTOMMENU_WINDOW_Y, CUSTOMMENU_WINDOW_SIZE_X, CUSTOMMENU_WINDOW_SIZE_Y);
	...
	for (int i = 0; i <= PC_RANDOM; i++)
	{
		...
		m_pButtons[i] = new ClassButton(i, sz, CUSTOMMENU_TOPLEFT_BUTTON_X, iYPos, CUSTOMMENU_BUTTON_SIZE_X, CUSTOMMENU_BUTTON_SIZE_Y, true);
		// RandomPC uses '0'
		if (i >= 1 && i <= 9)
		{
			sprintf(sz, "%d", i);
		...
		//m_pClassInfoPanel[i] = new CTransparentPanel( 255, CUSTOMMENU_WINDOW_X, CUSTOMMENU_WINDOW_Y, CUSTOMMENU_WINDOW_SIZE_X, CUSTOMMENU_WINDOW_SIZE_Y );
		m_pClassInfoPanel[i] = new CTransparentPanel(255, 0, 0, clientWide, CUSTOMMENU_WINDOW_SIZE_Y);
		...
		{
			textOffs = CUSTOMMENU_WINDOW_NAME_X;
		}
		...
		Label* pNameLabel = new Label("", textOffs, CUSTOMMENU_WINDOW_NAME_Y);
		...
		if (bShowClassGraphic)
		{
			for (int team = 0; team < 2; team++)
			{
				...
				m_pClassImages[team][i] = new CImageLabel(sz, 0, 0, CUSTOMMENU_WINDOW_TEXT_X, CUSTOMMENU_WINDOW_TEXT_Y);
				...
				pLabel->setPos((CUSTOMMENU_WINDOW_TEXT_X - pLabel->getWide()) / 2, yOut / 2);
			}
		}
		...
		m_pPlayers[i] = new Label("", textOffs, CUSTOMMENU_WINDOW_PLAYERS_Y);
		...
		TextPanel* pTextWindow = new TextPanel(cText, textOffs, CUSTOMMENU_WINDOW_TEXT_Y, (CUSTOMMENU_WINDOW_SIZE_X - textOffs) - 5, CUSTOMMENU_WINDOW_SIZE_Y - CUSTOMMENU_WINDOW_TEXT_Y);
	...
	m_pCancelButton = new CommandButton(gHUD.m_TextMessage.BufferedLocaliseTextString("#Menu_Cancel"), CUSTOMMENU_TOPLEFT_BUTTON_X, 0, CUSTOMMENU_BUTTON_SIZE_X, CUSTOMMENU_BUTTON_SIZE_Y);
	...
	for (int i = 0; i <= PC_RANDOM; i++)
	{
		...
		else
		{
			...
			else
			{
				...
				m_pButtons[i]->setPos(CUSTOMMENU_TOPLEFT_BUTTON_X, iYPos);
				iYPos += CUSTOMMENU_BUTTON_SIZE_Y + CUSTOMMENU_BUTTON_SPACER_Y;
...
bool CCustomMenu::SlotInput(int iSlot)
{
	if ((iSlot < 0) || (iSlot > 9))
		return false;
	if (!m_pButtons[iSlot])
...
void CCustomMenu::SetActiveInfo(int iInput)
{
