...
void CZombie::Precache()
{
	...
	for (i = 0; i < ARRAYSIZE(pIdleSoundsGrunt); i++)
		PRECACHE_SOUND((char*)pIdleSoundsGrunt[i]);

	for (i = 0; i < ARRAYSIZE(pAlertSounds); i++)
		PRECACHE_SOUND((char*)pAlertSounds[i]);

	for (i = 0; i < ARRAYSIZE(pAlertSoundsGrunt); i++)
		PRECACHE_SOUND((char*)pAlertSoundsGrunt[i]);
