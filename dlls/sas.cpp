...
int g_fSasQuestion; 				// true if an idle sas asked a question. Cleared when someone answers.
...
TYPEDESCRIPTION	CSas::m_SaveData[] =
{
	...
	//	DEFINE_FIELD( CSas, m_flLastEnemySightTime, FIELD_TIME ), // don't save, go to zero
		...
		//  DEFINE_FIELD( CShotgun, m_iShotgunShell, FIELD_INTEGER ),
			...
			DEFINE_FIELD(CSas, m_isFriendly, FIELD_BOOLEAN),
};

IMPLEMENT_SAVERESTORE(CSas, CSquadMonster);
...
//=========================================================
int CSas::IRelationship(CBaseEntity* pTarget)
{
...
{
	if (FOkToSpeak() && (g_fSasQuestion || RANDOM_LONG(0, 1)))
	{
		if (!g_fSasQuestion)
		{
			...
			{
			case 0: // check in
				SENTENCEG_PlayRndSz(ENT(pev), "SAS_CHECK", HGRUNT_SENTENCE_VOLUME, ATTN_NORM, 0, m_voicePitch);
				g_fSasQuestion = 1;
				break;
			case 1: // question
				SENTENCEG_PlayRndSz(ENT(pev), "SAS_QUEST", HGRUNT_SENTENCE_VOLUME, ATTN_NORM, 0, m_voicePitch);
				g_fSasQuestion = 2;
				break;
			case 2: // statement
				SENTENCEG_PlayRndSz(ENT(pev), "SAS_IDLE", HGRUNT_SENTENCE_VOLUME, ATTN_NORM, 0, m_voicePitch);
				break;
			}
		}
		else
		{
			switch (g_fSasQuestion)
			{
			case 1: // check in
				SENTENCEG_PlayRndSz(ENT(pev), "SAS_CLEAR", HGRUNT_SENTENCE_VOLUME, ATTN_NORM, 0, m_voicePitch);
				break;
			case 2: // question 
				SENTENCEG_PlayRndSz(ENT(pev), "SAS_ANSWER", HGRUNT_SENTENCE_VOLUME, ATTN_NORM, 0, m_voicePitch);
				break;
			}
			g_fSasQuestion = 0;
		}
...
void CSas::HandleAnimEvent(MonsterEvent_t* pEvent)
{
	...
	case HGRUNT_AE_CAUGHT_ENEMY:
	{
		if (FOkToSpeak())
		{
			SENTENCEG_PlayRndSz(ENT(pev), "SAS_ALERT", HGRUNT_SENTENCE_VOLUME, GRUNT_ATTN, 0, m_voicePitch);
			JustSpoke();
		}
...
Schedule_t	slSasCombatFail[] =
{
	{
		tlSasCombatFail,
		ARRAYSIZE(tlSasCombatFail),
		bits_COND_CAN_RANGE_ATTACK1 |
		bits_COND_CAN_RANGE_ATTACK2,
		0,
		"Grunt Combat Fail"
...
Schedule_t	slSasVictoryDance[] =
{
	{
		tlSasVictoryDance,
		ARRAYSIZE(tlSasVictoryDance),
		bits_COND_NEW_ENEMY |
...
// the grunt to attack.
//=========================================================
Task_t tlSasEstablishLineOfFire[] =
{
...
Schedule_t slSasEstablishLineOfFire[] =
{
	{
		tlSasEstablishLineOfFire,
		ARRAYSIZE(tlSasEstablishLineOfFire),
		bits_COND_NEW_ENEMY |
...
Schedule_t	slSasFoundEnemy[] =
{
	{
		tlSasFoundEnemy,
		ARRAYSIZE(tlSasFoundEnemy),
		bits_COND_HEAR_SOUND,
...
Schedule_t	slSasCombatFace[] =
{
	{
		tlSasCombatFace1,
		ARRAYSIZE(tlSasCombatFace1),
		bits_COND_NEW_ENEMY |
		bits_COND_ENEMY_DEAD |
		bits_COND_CAN_RANGE_ATTACK1 |
		bits_COND_CAN_RANGE_ATTACK2,
...
Schedule_t	slSasSignalSuppress[] =
{
	{
		tlSasSignalSuppress,
		ARRAYSIZE(tlSasSignalSuppress),
		bits_COND_ENEMY_DEAD |
		bits_COND_LIGHT_DAMAGE |
		bits_COND_HEAVY_DAMAGE |
		bits_COND_HEAR_SOUND |
...
Schedule_t	slSasSuppress[] =
{
	{
		tlSasSuppress,
		ARRAYSIZE(tlSasSuppress),
		bits_COND_ENEMY_DEAD |
		bits_COND_LIGHT_DAMAGE |
		bits_COND_HEAVY_DAMAGE |
		bits_COND_HEAR_SOUND |
...
Schedule_t	slSasWaitInCover[] =
{
	{
		tlSasWaitInCover,
		ARRAYSIZE(tlSasWaitInCover),
		bits_COND_NEW_ENEMY |
		bits_COND_HEAR_SOUND |
		bits_COND_CAN_RANGE_ATTACK1 |
		bits_COND_CAN_RANGE_ATTACK2 |
...
Schedule_t	slSasTakeCover[] =
{
	{
		tlSasTakeCover1,
		ARRAYSIZE(tlSasTakeCover1),
		0,
		0,
		"TakeCover"
	},
...
Schedule_t	slSasGrenadeCover[] =
{
	{
		tlSasGrenadeCover1,
		ARRAYSIZE(tlSasGrenadeCover1),
		0,
		0,
		"GrenadeCover"
	},
...
Schedule_t	slSasTossGrenadeCover[] =
{
	{
		tlSasTossGrenadeCover1,
		ARRAYSIZE(tlSasTossGrenadeCover1),
		0,
		0,
		"TossGrenadeCover"
	},
...
//=========================================================
Task_t	tlSasTakeCoverFromBestSound[] =
{
...
Schedule_t	slSasTakeCoverFromBestSound[] =
{
	{
		tlSasTakeCoverFromBestSound,
		ARRAYSIZE(tlSasTakeCoverFromBestSound),
		0,
		0,
...
Schedule_t slSasHideReload[] =
{
	{
		tlSasHideReload,
		ARRAYSIZE(tlSasHideReload),
		bits_COND_HEAVY_DAMAGE |
		bits_COND_HEAR_SOUND,
...
Schedule_t	slSasSweep[] =
{
	{
		tlSasSweep,
		ARRAYSIZE(tlSasSweep),
...
Schedule_t	slSasRangeAttack1A[] =
{
	{
		tlSasRangeAttack1A,
		ARRAYSIZE(tlSasRangeAttack1A),
		bits_COND_NEW_ENEMY |
		bits_COND_ENEMY_DEAD |
		bits_COND_HEAVY_DAMAGE |
...
Schedule_t	slSasRangeAttack1B[] =
{
	{
		tlSasRangeAttack1B,
		ARRAYSIZE(tlSasRangeAttack1B),
		bits_COND_NEW_ENEMY |
		bits_COND_ENEMY_DEAD |
		bits_COND_HEAVY_DAMAGE |
		bits_COND_ENEMY_OCCLUDED |
...
Schedule_t	slSasRangeAttack2[] =
{
	{
		tlSasRangeAttack2,
		ARRAYSIZE(tlSasRangeAttack2),
		0,
		0,
		"RangeAttack2"
	},
...
Schedule_t	slSasRepel[] =
{
	{
		tlSasRepel,
		ARRAYSIZE(tlSasRepel),
		bits_COND_SEE_ENEMY |
		bits_COND_NEW_ENEMY |
		bits_COND_LIGHT_DAMAGE |
		bits_COND_HEAVY_DAMAGE |
...
Schedule_t	slSasRepelAttack[] =
{
	{
		tlSasRepelAttack,
		ARRAYSIZE(tlSasRepelAttack),
		bits_COND_ENEMY_OCCLUDED,
		0,
		"Repel Attack"
	},
...
Schedule_t	slSasRepelLand[] =
{
	{
		tlSasRepelLand,
		ARRAYSIZE(tlSasRepelLand),
		bits_COND_SEE_ENEMY |
		bits_COND_NEW_ENEMY |
		bits_COND_LIGHT_DAMAGE |
		bits_COND_HEAVY_DAMAGE |
...
DEFINE_CUSTOM_SCHEDULES(CSas)
{
	slSasFail,
		...
		slSasTakeCover,
		slSasGrenadeCover,
		slSasTossGrenadeCover,
		slSasTakeCoverFromBestSound,
		slSasHideReload,
		slSasSweep,
		slSasRangeAttack1A,
		slSasRangeAttack1B,
...
IMPLEMENT_CUSTOM_SCHEDULES(CSas, CSquadMonster);
	...
	if (HasConditions(bits_COND_HEAR_SOUND))
	{
		...
		if (pSound)
		{
			if (pSound->m_iType & bits_SOUND_DANGER)
			{
				...
				if (FOkToSpeak())
				{
					SENTENCEG_PlayRndSz(ENT(pev), "SAS_GREN", HGRUNT_SENTENCE_VOLUME, GRUNT_ATTN, 0, m_voicePitch);
					JustSpoke();
				}
	...
	case MONSTERSTATE_COMBAT:
	{
					...
					if (FOkToSpeak())// && RANDOM_LONG(0,1))
					{
						...
						//else if ((m_hEnemy != NULL) &&
							...
							{ 
								...
								// monster
								SENTENCEG_PlayRndSz(ENT(pev), "SAS_MONST", HGRUNT_SENTENCE_VOLUME, GRUNT_ATTN, 0, m_voicePitch);
		...
		else if (HasConditions(bits_COND_LIGHT_DAMAGE))
		{
			...
			if (iPercent <= 90 && m_hEnemy != NULL)
			{
				...
				if (FOkToSpeak()) // && RANDOM_LONG(0,1))
				{
					//SENTENCEG_PlayRndSz( ENT(pev), "SAS_COVER", HGRUNT_SENTENCE_VOLUME, GRUNT_ATTN, 0, m_voicePitch);
		...
		else if (HasConditions(bits_COND_ENEMY_OCCLUDED))
		{
			if (HasConditions(bits_COND_CAN_RANGE_ATTACK2) && OccupySlot(bits_SLOTS_HGRUNT_GRENADE))
			{
				...
				if (FOkToSpeak())
				{
					SENTENCEG_PlayRndSz(ENT(pev), "SAS_THROW", HGRUNT_SENTENCE_VOLUME, GRUNT_ATTN, 0, m_voicePitch);
					JustSpoke();
				}
				return GetScheduleOfType(SCHED_RANGE_ATTACK2);
			}
			else if (OccupySlot(bits_SLOTS_HGRUNT_ENGAGE))
			{
				...
				if (FOkToSpeak())// && RANDOM_LONG(0,1))
				{
					//SENTENCEG_PlayRndSz( ENT(pev), "SAS_CHARGE", HGRUNT_SENTENCE_VOLUME, GRUNT_ATTN, 0, m_voicePitch);
			...
			else
			{
				...
				if (FOkToSpeak() && RANDOM_LONG(0, 1))
				{
					SENTENCEG_PlayRndSz(ENT(pev), "SAS_TAUNT", HGRUNT_SENTENCE_VOLUME, GRUNT_ATTN, 0, m_voicePitch);
					JustSpoke();
				}
...
Schedule_t* CSas::GetScheduleOfType(int Type)
{
	...
	case SCHED_TAKE_COVER_FROM_ENEMY:
	{
		if (InSquad())
		{
			if (g_iSkillLevel == SKILL_HARD && HasConditions(bits_COND_CAN_RANGE_ATTACK2) && OccupySlot(bits_SLOTS_HGRUNT_GRENADE))
			{
				...
				return slSasTossGrenadeCover;
			}
			else
			{
				return &slSasTakeCover[0];
			}
		}
		else
		{
	...
	{
		return &slSasTakeCoverFromBestSound[0];
	}
	...
	{
		return &slSasEstablishLineOfFire[0];
	}
	break;
	case SCHED_RANGE_ATTACK1:
	{
		...
		if (m_fStanding)
			return &slSasRangeAttack1B[0];
		else
			return &slSasRangeAttack1A[0];
	}
	case SCHED_RANGE_ATTACK2:
	{
		return &slSasRangeAttack2[0];
	}
	case SCHED_COMBAT_FACE:
	{
		return &slSasCombatFace[0];
	}
	case SCHED_GRUNT_WAIT_FACE_ENEMY:
	{
		return &slSasWaitInCover[0];
	}
	...
	{
		return &slSasHideReload[0];
	}
	case SCHED_GRUNT_FOUND_ENEMY:
	{
		return &slSasFoundEnemy[0];
	}
	...
	case SCHED_GRUNT_SUPPRESS:
	{
		...
		if (m_hEnemy->IsPlayer() && m_fFirstEncounter)
		{
			...
			return &slSasSignalSuppress[0];
		}
		else
		{
		*/
			return &slSasSuppress[0];
		//}
	}
	case SCHED_FAIL:
	{
	...
	case SCHED_GRUNT_REPEL_ATTACK:
	{
		if (pev->velocity.z > -128)
			pev->velocity.z -= 32;
		return &slSasRepelAttack[0];
	}
	case SCHED_GRUNT_REPEL_LAND:
	{
		return &slSasRepelLand[0];
	}
	default:
	{
...
const char* CDeadSas::m_szPoses[] = { "deadstomach", "deadside", "deadsitting" };
...
void CDeadSas::Spawn(void)
{
	...
	if (pev->sequence == -1)
	{
		ALERT(at_debug, "Dead sas with bad pose\n");
	}
