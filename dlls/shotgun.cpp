...
void CShotgun::PrimaryAttack()
{
	...
	//if (m_iClip != 0)
		m_flPumpTime = gpGlobals->time + 0.55;
...
void CShotgun::Reload()
{
	...
	if (m_fInSpecialReload == 0)
	{
		...
		m_pPlayer->m_flNextAttack = UTIL_WeaponTimeBase() + 0.57;
		m_flTimeWeaponIdle = UTIL_WeaponTimeBase() + 0.57;
		m_flNextPrimaryAttack = GetNextAttackDelay(0.57);
		m_flNextSecondaryAttack = UTIL_WeaponTimeBase() + 0.57;
		return;
	}
	...
	if (m_flTimeWeaponIdle <  UTIL_WeaponTimeBase() )
	{
		...
		else if (m_fInSpecialReload != 0)
		{
			...
			else
			{
				...
				// play cocking sound - its handled in model
