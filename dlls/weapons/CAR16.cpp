...
class CAR16AmmoClip : public CBasePlayerAmmo
{
	...
	{
		int bResult = (pOther->GiveAmmo(AMMO_AR16CLIP_GIVE, "556", M249_MAX_CARRY) != -1);
		if (bResult)
		{
...
class CAR16Chainammo : public CBasePlayerAmmo
{
	...
	BOOL AddAmmo(CBaseEntity* pOther) override
	{
		int bResult = (pOther->GiveAmmo(AMMO_CHAINBOX_GIVE, "556", M249_MAX_CARRY) != -1);
		if (bResult)
		{
			EMIT_SOUND(ENT(pev), CHAN_ITEM, "items/9mmclip1.wav", 1, ATTN_NORM);
		}
